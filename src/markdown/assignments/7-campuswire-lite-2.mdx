...
Refer to the [example in offical docs](https://reactrouter.com/docs/en/v6/examples/basic) for how to set it up.

To install `react-router-dom`, simply run `npm install react-router-dom`
...
- a link that, once clicked, sends user to the `/login` page (the `<Link />` component from `react-router-dom` should be helpful, refer to the [link](https://reactrouter.com/docs/en/v6/api#link))

Also, there are a few requirements regarding sign up:
- if the user successfully signed up, you should redirect them back to the home page `/` (the `useNavigate` function from `react-router-dom` is helpful here, here's the [link](https://reactrouter.com/docs/en/v6/api#usenavigate))
...
- a list of questions where each question should includes (you need to send a `GET` request to `/api/questions`)
  ...
  - the author of the question
  - the answer of the question
  - a form for the user to submit an answer (the submit button will need to make a `POST` request to `/api/questions/answer`). The user should be able to **update answers afterwards however many times** he/ she wants.
